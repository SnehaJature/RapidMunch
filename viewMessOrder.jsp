...
String sql="select mess_order1.date,registration.name,registration.mobile,registration.city,registration.id ,mess_order1.status from mess_order1 inner join registration on mess_order1.user_id=registration.id order by mess_order1.id desc";
...
<td><a href="viewMessOrder.jsp?user_id=<%=rs.getString(5) %>&date=<%=rs.getString(1) %>">View Order</a></td>
<%if(rs.getString(6).equals("pending")) {%>
<td><a id="status" onclick="return myFunction()" href="status_update_mess.jsp?user_id=<%=rs.getString(5) %>&date=<%=rs.getString(1) %>&status=<%=rs.getString(6) %>" style="color:red;"><%=rs.getString(6) %></td>
<%}else{ %>
...
String sql1="select *from mess_order where user_id=? and date=?";
...
String sql3="select *from mess_order1 where user_id=? and date=?";
