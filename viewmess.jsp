...
<%-- <td><a href="adminviewmessdelete.jsp?id=<%=rs.getString(1)%>">Delete</a></td> --%>
<%if(rs.getString(6)!= null && rs.getString(6).equals("hide")){ %>
<td><a href="adminviewmessstatus.jsp?mess_id=<%=rs.getString(1)%>&status=<%=rs.getString(6)%>" class="btn btn-info btn-xs">Show</a></td>
<%}else{ %>
<td><a href="adminviewmessstatus.jsp?mess_id=<%=rs.getString(1)%>&status=<%=rs.getString(6)%>" class="btn btn-primary btn-xs">Hide</a></td>
<%} %>
</tr>
